...
bool KerasLayerActivation::Apply(Tensor* in, Tensor* out) {
    ...
    *out = *in;
    ...
    case kSoftmax:
    {
        float data_exp[out->data_.size()];
        float sum_data_exp = 0.0;
        for (size_t i = 0; i < out->data_.size(); i++) {
            data_exp[i] = std::exp(out->data_[i]);
            sum_data_exp += data_exp[i];
        }
        for (size_t i = 0; i < out->data_.size(); i++) {
            out->data_[i] = data_exp[i] / sum_data_exp;
        }
    }break;
    case kHardSigmoid:
        for (size_t i = 0; i < out->data_.size(); i++) {
...
bool KerasLayerTimeDistributed::LoadLayer(std::ifstream* file) {
...
bool KerasLayerTimeDistributed::Apply(Tensor* in, Tensor* out) {

    KASSERT(in, "Invalid input");
    KASSERT(out, "Invalid output");
    KASSERT(in->dims_.size() <= 3, "Invalid input dimensions");

    if (in->dims_.size() == 3) {
        KASSERT(in->dims_[2] == weights_.dims_[0], "Dimension mismatch %d %d",
                in->dims_[2], weights_.dims_[0]);
    }
    ...
    Tensor tmp(in_0, weights_.dims_[1]);

    // Initialize tmp Tensor
    for (int i = 0; i < in_0; i++) {
        for (int j = 0; j < weights_.dims_[1]; j++) {
            for (int k = 0; k < in_1; k++) {
                if (in->dims_.size() == 3) {
                    tmp(i, j) += 0.0;
                }else {
                    tmp(i, j) += 0.0;
    ...
    for (int i = 0; i < in_0; i++) {
        for (int j = 0; j < weights_.dims_[1]; j++) {
            for (int k = 0; k < in_1; k++) {
                if (in->dims_.size() == 3) {
                    tmp(i, j) += (*in)(0, i, k) * weights_(k, j);
                }else {
                    tmp(i, j) += (*in)(i, k) * weights_(k, j);
                }
            }
        }
    }
    ...
    for(int i = 0; i < in_0; i++)
    {
        for (int j = 0; j < biases_.dims_[0]; j++) {
            tmp(i,j) += biases_(j);
        }
    }
...
bool KerasModel::LoadModel(const std::string& filename) {
    ...
    for (unsigned int i = 0; i < num_layers; i++) {
        ...
        case kEmbedding:
            ...
            break;
        case kTimeDistributed:
            layer = new KerasLayerTimeDistributed();
            break;
        default:
            break;
        }
